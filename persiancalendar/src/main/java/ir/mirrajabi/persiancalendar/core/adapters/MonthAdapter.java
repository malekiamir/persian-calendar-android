...
public class MonthAdapter extends RecyclerView.Adapter<MonthAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(MonthAdapter.ViewHolder holder, int position) {
        ...
        mCalendarHandler.setHeadersFontSize(80);
        mCalendarHandler.setDaysFontSize(80);
        if (!isPositionHeader(position)) {
            if (position - 7 - mFirstDayOfWeek >= 0) {
                ...
                if (mDays.get(position - 7 - mFirstDayOfWeek).isToday()) {
                    ...
                    holder.mNum.setTextColor(Color.rgb(73,98,41));
                } else {
                ...
                if ( (mDays.get(position - 7 - mFirstDayOfWeek).getPersianDate().getDayOfMonth()
                        > (today.getDayOfMonth()) &&
                        !(mDays.get(position - 7 - mFirstDayOfWeek).getPersianDate().getMonth()
                                < today.getMonth()) &&
                        !(mDays.get(position - 7 - mFirstDayOfWeek).getPersianDate().getYear()
                                < today.getYear()) )
                        ||
                        (mDays.get(position - 7 - mFirstDayOfWeek).getPersianDate().getMonth()
                                > today.getMonth() &&
                        ...
                        (mDays.get(position - 7 - mFirstDayOfWeek).getPersianDate().getYear()
                                > today.getYear())) {
                    holder.dayBg.setVisibility(View.GONE);
                    holder.mNum.setTextColor(Color.rgb(73,98,41));
                }
            } else {
                holder.mToday.setVisibility(View.GONE);
